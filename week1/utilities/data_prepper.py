...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        hits = response and response.get('hits')
        hits = hits.get('hits') if hits else None
        ...
        for hit in hits:
            ...
            for feat in hit["fields"]["_ltrlog"][0]["log_entry"]:
                name, value = feat.get("name"), feat.get("value", 0)
                feature_results.setdefault(name, []).append(value)
